<?php
/**
 * Plugin Name: <b style="color: #FF8C00;">Biblio-addons-my_MM
 * Plugin URI: -
 * Description: Panel for addons to the cataloguing main page, Burmese version.
 * Version: 1.0.0-my
...
$plugins->register('bibliography_init', function(){
    ...
    <div class="floating-panel_MM" id="Helper_MM">
        ...
        <div class="panel-content_MM">
            ...
            <button onclick="openEDDC()">E-DDC</button>
            <button onclick="openLOC()">LOC</button>

             <hr>
             <button onclick="openQuickNotes()">အမြန်မှတ်စုများ</button>
             
            <button onclick="openChecklist()">✓ စစ်ဆေးရမည့်စာရင်း</button>
        </div>
    </div>
    ...
    <script>
        ...
        function openMLA() {
            window.open(
                " ../plugins/mla_subject-headings/index.html https://zawaunghtut.github.io/MLA_Subject_Heading/",
                "mlaHeadings",
                "width=800,height=600,toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes"
            );
        }
